...
for j in range(ensemble_size):
    if False:
        # Example 0: Uniform distribution
        p0x = np.random.uniform(size=num_landmarks)
        p0y = np.random.uniform(size=num_landmarks)
    ...
    if True:
        ...
        # perturb the ensemble
        p0_np = utils.sample_normal(num_landmarks, 0, 1)
        p0 = torch.tensor(np.random.uniform(-10, 10) * p0_np, dtype=torch_dtype, requires_grad=True)
        pe.append(p0)

q1 /= ensemble_size
plot_q(filename="q1_truth", q0=q0, q1=q1)

# 3) set up and run Kalman filter
ke = EnsembleKalmanFilter(ensemble_size, q0, q1, log_dir=log_dir)
ke.dump_parameters()
